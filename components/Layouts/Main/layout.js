...
import {useCheckExpireEffect} from "../../../src/customEffects/useCheckExpireEffect";

export const Layout = ({children}) => {

    const {data: session, status} = useSession()
    useSetLocalStorageSignUpData()
    useCheckExpireEffect(session,status)
        ...
        (
            <div className={'w-full h-[100vh] flex flex-col justify-start pt-80 items-center bg-black'}>
                <div className={'flex flex-row items-center justify-center gap-4 w-full h-10'}>
                    ...
                    <div
                        className={'w-10 h-10 rounded-full bg-gradient-to-tr from-skin-theme-400 to-skin-theme-600 ring-1 ring-white animate-spin'}></div>
